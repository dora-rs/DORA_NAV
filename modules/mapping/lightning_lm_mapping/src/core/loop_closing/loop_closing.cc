...
#include <yaml-cpp/yaml.h>  // DORA: 添加 yaml-cpp 支持
...
// #include "io/yaml_io.h"  // DORA: 不存在的文件
...
void LoopClosing::Init(const std::string yaml_path) {
    ...
    if (!yaml_path.empty()) {
        // DORA: 使用标准 yaml-cpp 替代 YAML_IO
        auto yaml = YAML::LoadFile(yaml_path);
        options_.loop_kf_gap_ = yaml["loop_closing"]["loop_kf_gap"].as<int>();
        options_.min_id_interval_ = yaml["loop_closing"]["min_id_interval"].as<int>();
        options_.closest_id_th_ = yaml["loop_closing"]["closest_id_th"].as<int>();
        options_.max_range_ = yaml["loop_closing"]["max_range"].as<double>();
        options_.ndt_score_th_ = yaml["loop_closing"]["ndt_score_th"].as<double>();
        options_.with_height_ = yaml["loop_closing"]["with_height"].as<bool>();
    }
