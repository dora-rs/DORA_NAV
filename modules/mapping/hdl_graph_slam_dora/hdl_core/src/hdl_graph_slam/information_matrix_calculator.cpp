...
InformationMatrixCalculator::InformationMatrixCalculator(const YAML::Node& config) {
  use_const_inf_matrix = config["use_const_inf_matrix"].as<bool>(false);
  const_stddev_x = config["const_stddev_x"].as<double>(0.5);
  const_stddev_q = config["const_stddev_q"].as<double>(0.1);

  var_gain_a = config["var_gain_a"].as<double>(20.0);
  min_stddev_x = config["min_stddev_x"].as<double>(0.1);
  max_stddev_x = config["max_stddev_x"].as<double>(5.0);
  min_stddev_q = config["min_stddev_q"].as<double>(0.05);
  max_stddev_q = config["max_stddev_q"].as<double>(0.2);
  fitness_score_thresh = config["fitness_score_thresh"].as<double>(0.5);
}
