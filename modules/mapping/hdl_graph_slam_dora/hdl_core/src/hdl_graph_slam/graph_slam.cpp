...
// G2O_USE_OPTIMIZATION_LIBRARY(cholmod)  // cholmod not compiled in this g2o build
// G2O_USE_OPTIMIZATION_LIBRARY(csparse)  // csparse not compiled in this g2o build
...
int GraphSLAM::optimize(int num_iterations) {
  ...
  std::cout << "time: " << boost::format("%.3f") % elapsed_sec << "[sec]" << std::endl;
