...
KeyFrame::KeyFrame(double stamp, const Eigen::Isometry3d& odom, double accum_distance, const pcl::PointCloud<PointT>::ConstPtr& cloud) : stamp(stamp), odom(odom), accum_distance(accum_distance), cloud(cloud), node(nullptr) {}

KeyFrame::KeyFrame(const std::string& directory, g2o::HyperGraph* graph) : stamp(0.0), odom(Eigen::Isometry3d::Identity()), accum_distance(-1), cloud(nullptr), node(nullptr) {
...
void KeyFrame::save(const std::string& directory) {
  ...
  ofs << "stamp " << std::fixed << std::setprecision(9) << stamp << "\n";
...
bool KeyFrame::load(const std::string& directory, g2o::HyperGraph* graph) {
  ...
  if(node_id < 0) {
    std::cerr << "[KeyFrame] Invalid node id!!" << std::endl;
    std::cerr << "[KeyFrame] Directory: " << directory << std::endl;
    return false;
  }

  if(graph->vertices().find(node_id) == graph->vertices().end()) {
    std::cerr << "[KeyFrame] Vertex ID=" << node_id << " does not exist!!" << std::endl;
    return false;
  }

  node = dynamic_cast<g2o::VertexSE3*>(graph->vertices()[node_id]);
  if(node == nullptr) {
    std::cerr << "[KeyFrame] Failed to downcast!!" << std::endl;
    return false;
  }
