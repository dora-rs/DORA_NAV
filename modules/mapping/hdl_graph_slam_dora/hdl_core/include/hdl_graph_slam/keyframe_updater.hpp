...
public:
   ...
   */
  KeyframeUpdater(const YAML::Node& config) : is_first(true), prev_keypose(Eigen::Isometry3d::Identity()) {
    keyframe_delta_trans = config["keyframe_delta_trans"].as<double>(2.0);
    keyframe_delta_angle = config["keyframe_delta_angle"].as<double>(2.0);
