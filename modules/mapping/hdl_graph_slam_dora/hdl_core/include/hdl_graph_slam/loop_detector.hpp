...
public:
   ...
   */
  LoopDetector(const YAML::Node& config) {
    distance_thresh = config["distance_thresh"].as<double>(5.0);
    accum_distance_thresh = config["accum_distance_thresh"].as<double>(8.0);
    distance_from_last_edge_thresh = config["min_edge_interval"].as<double>(5.0);

    fitness_score_max_range = config["fitness_score_max_range"].as<double>(std::numeric_limits<double>::max());
    fitness_score_thresh = config["fitness_score_thresh"].as<double>(0.5);

    registration = select_registration_method(config);
...
private:
  ...
  Loop::Ptr matching(const std::vector<KeyFrame::Ptr>& candidate_keyframes, const KeyFrame::Ptr& new_keyframe, hdl_graph_slam::GraphSLAM& graph_slam) {
    ...
    std::cout << "best_score: " << boost::format("%.3f") % best_score << "    time: " << boost::format("%.3f") % elapsed_sec << "[sec]" << std::endl;
