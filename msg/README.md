...
数据格式 payload 的长度为：**16 字节头部 + point_count × 16 字节点记录**

16 字节头部

| 字节偏移 | 大小 | 类型      | 字段      | 含义                 |
| -------: | ---: | --------- | --------- | -------------------- |
|        0 |    4 | `uint32`  | sequence  | 点云帧序号           |
|        4 |    4 | padding   | reserved  | 当前填零             |
|        8 |    8 | `float64` | timestamp | ROS 时间戳，单位为秒 |
...
该头部没有 magic、协议版本、点数或 point stride 字段。接收端需要根据 数据帧总长度计算点数：

```text
point_count = (payload_length - 16) / 16
```

**16 字节点记录**

每个点由连续 4 个 `float32` 组成：
...
`intensity` 输入支持 ROS PointField 的 `FLOAT32`、`UINT8` 和 `UINT16`，后两者会
转换成 `float32`。缺少 intensity 或其类型不受支持时发布值为 `0.0`。 

最终的“16 字节头部 + XYZI 点数组”同样通过一维 Arrow `uint8` 数组发布到 Dora：

```python
node.send_output(
    "pointcloud",
    pa.array(np.frombuffer(payload_bytes, dtype=np.uint8)),
)
```

### 1.7 XYZITRR 点云数据格式

`src/nclt_pointcloud_imu_pub_dora_node.py` 发布两个 Dora output：

- `pointcloud`：FLIO v1 Standard 二进制点云。
- `imu`：UTF-8 编码的 JSON。

两个 payload 都通过一维 Arrow `uint8` 数组发送：

```python
node.send_output(
    output_id,
    pa.array(np.frombuffer(payload, dtype=np.uint8)),
)
```
...
```text
40 字节头部 + point_count × 24 字节点记录
```

头部对应 Python `struct` 格式：

```python
"<4sBBBBHHIdIIB7x"
```

| 字节偏移 | 大小 | 类型      | 字段          | 当前值或含义         |
| -------: | ---: | --------- | ------------- | -------------------- |
|        0 |    4 | `char[4]` | magic         | `FLIO`               |
|        4 |    1 | `uint8`   | version       | `1`                  |
|        5 |    1 | `uint8`   | endianness    | `0`，小端            |
|        6 |    1 | `uint8`   | format_kind   | `1`，Standard        |
|        7 |    1 | `uint8`   | field_flags   | `0x03`               |
|        8 |    2 | `uint16`  | header_size   | `40`                 |
|       10 |    2 | `uint16`  | point_stride  | `24`                 |
|       12 |    4 | `uint32`  | sequence      | 点云帧序号           |
|       16 |    8 | `float64` | timestamp     | ROS 时间戳，单位为秒 |
|       24 |    4 | `uint32`  | point_count   | 过滤后的有效点数     |
|       28 |    4 | `uint32`  | payload_bytes | `point_count × 24`   |
|       32 |    1 | `uint8`   | dimensions    | `3`，表示 XYZ        |
|       33 |    7 | padding   | reserved      | 填零                 |
...
|        8 |    4 | `float32` | `z`                  |
|       12 |    4 | `float32` | `intensity`          |
|       16 |    4 | `float32` | `time`，点相对时间   |
|       20 |    2 | `uint16`  | `ring`，激光线束编号 |
|       22 |    2 | `uint16`  | `reserved`，当前填零 |

输入的 点云必须包含 `x`、`y`、`z`、`time` 和 `ring`。其中 XYZ、time 为 `FLOAT32`，ring 为 `UINT16`。`intensity` 可选，但存在时必须为 `FLOAT32`；不存在时发布值为 `0.0`。编码器会删除 XYZ 含 NaN 或 Inf 的点，如果整帧没有有限 XYZ 点则报错并停止发布该帧。
...
当前实现以 `msg/nav_msgs_type` 下的头文件为准，头文件采用 `*_message.hpp` 小写下划线命名，例如 `base_message.hpp`、`pose_message.hpp`、`twist_message.hpp`、`odom_message.hpp`、`path_message.hpp`。消息类型名仍保留 `PoseMessage`、`OdomMessage` 等 C++ 类型名。当前已包含轻量导航消息和带协方差/传感器消息：

| 文件 | 类型 | 用途 |
| --- | --- | --- |
| `base_message.hpp` | `Time`, `HeaderMessage`, `Vector3`, `QuaternionMessage`, `PointMessage` | 基础字段类型 |
| `pose_message.hpp` | `PoseMessage`, `PoseStampedMessage` | 轻量位姿与带 header 位姿 |
| `twist_message.hpp` | `TwistDataMessage`, `TwistMessage` | 轻量速度量与带 header 控制/速度消息 |
| `odom_message.hpp` | `OdomMessage` | 轻量里程计，不带协方差 |
| `path_message.hpp` | `PathMessage` | 路径 |
| `pose_with_covariance_message.hpp` | `PoseWithCovarianceMessage`, `PoseWithCovarianceStampedMessage` | 带 6x6 协方差位姿 |
| `twist_with_covariance_message.hpp` | `TwistWithCovarianceMessage` | 带 6x6 协方差速度 |
| `odometry_with_covariance_message.hpp` | `OdometryWithCovarianceMessage` | 接近 ROS Odometry 结构的带协方差里程计 |
| `imu_message.hpp` | `ImuMessage` | IMU：姿态、角速度、线加速度及三组 3x3 协方差 |
| `nav_sat_fix_message.hpp` | `NavSatStatusMessage`, `NavSatFixMessage` | GNSS：经纬高、定位状态、位置协方差 |

实现采用“基础类型先定义 JSON 转换、上层消息组合基础类型”的方式。例如 `PointMessage` 定义 `x/y/z` 的转换，`PoseMessage` 再定义 `position/orientation` 的转换。不要在 `NLOHMANN_DEFINE_TYPE_INTRUSIVE` 中写 `position.x`、`header.stamp.sec` 这类点路径；nlohmann/json 宏会把它们当成字段名字符串处理，不能自动生成嵌套 JSON。正确做法是：

```C++
NLOHMANN_DEFINE_TYPE_INTRUSIVE(Time, sec, nanosec)
NLOHMANN_DEFINE_TYPE_INTRUSIVE(HeaderMessage, stamp, frame_id, seq)
NLOHMANN_DEFINE_TYPE_INTRUSIVE(PointMessage, x, y, z)
NLOHMANN_DEFINE_TYPE_INTRUSIVE(QuaternionMessage, x, y, z, w)
NLOHMANN_DEFINE_TYPE_INTRUSIVE(PoseMessage, position, orientation)
NLOHMANN_DEFINE_TYPE_INTRUSIVE(PoseStampedMessage, header, pose)
```

工程内提供了 `msg/nav_msgs_type/json.hpp` 包装头，实际仍依赖系统或构建环境提供 `<nlohmann/json.hpp>`。
